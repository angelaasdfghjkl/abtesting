...
def get_2_sample_df(a, b):
    ...
    nb = len(b)
    n = std_err**4
    sa= (std_a**2/na)**2/(na-1)
    sb= (std_b**2/nb)**2/(nb-1)
    return round(n/(sa+sb))
...
def data_to_num_list(s):
 ...
 #t_test for clean data of "time to complete"
a_t_list = data_to_num_list(a)
b_t_list = data_to_num_list(b)
print(get_t_score(a_t_list, b_t_list)) #t-score
print(perform_2_sample_t_test(a_t_list, b_t_list)) #p-value

#chi2_test for clean data of "return rate":
a_chi_list = data_to_num_list(a_count)
b_chi_list = data_to_num_list(b_count)
c1_observed_grid = [a_chi_list, b_chi_list]
print(chi2_value(c1_observed_grid))
#chi^2 value of the expected return rate
print(perform_chi2_homogeneity_test(c1_observed_grid))  # p-value
